...
def gerar_questao(req QuestaoInput)
    return {
        enunciado fSobre o tema '{req.tema}', analise a afirmativa abaixo...,
        alternativas [A) Alternativa 1, B) Alternativa 2, C) Alternativa 3, D) Alternativa 4],
        gabarito B
    }
...
def reflexion(req ReflexionInput)
    return {
        resposta_replanejada fPlano ajustado Reforçar abordagem de '{req.contexto}' com esquemas visuais e casos simulados.
    }

@app.post(summary)
def summary(req SummaryInput)
    return {
        resumo fResumo (nível {req.nivel}) {req.texto[120]}...
    }

@app.post(feedback)
def feedback(req FeedbackInput)
    return {
        comentario fAnálise do desempenho {req.desempenho or 'não informado'}. Sua resposta contém bons argumentos, mas falta fundamentação legal.
    }

@app.post(memoria)
def memoria(req MemoriaInput)
    return {
        status fMemória salva {req.chave} = {req.valor}
    }
